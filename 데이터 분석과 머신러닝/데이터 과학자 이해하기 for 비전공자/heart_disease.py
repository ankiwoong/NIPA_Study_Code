...
def load_dataset():
    data = pd.read_csv(
        "D:\\Code\\Study\\NIPA_Study_Code\\데이터 분석과 머신러닝\\데이터 과학자 이해하기 for 비전공자\\data\\processed.cleveland.data",
        dtype=columns_type,
        names=columns,
    )
    ...
    for c in data.columns[:-1]:
        data[c] = data[c].apply(
            lambda x: data[data[c] != "?"][c].astype(float).mean() if x == "?" else x
        )
...
def cleansing_categorical(indices_categorical_columns):
    categorical_pipline = Pipeline(
        steps=[
            (
                "select",
                FunctionTransformer(lambda data: data[:, indices_categorical_columns]),
            ),
...
def cleansing_numeric(indices_numeric_columns):
    numeric_pipline = Pipeline(
        steps=[
            (
                "select",
                FunctionTransformer(lambda data: data[:, indices_numeric_columns]),
            ),
                ...
                (
                    ...
                    FeatureUnion(
                        transformer_list=[
                            (
                                "categorical",
                                cleansing_categorical(indices_categorical_columns),
                            ),
